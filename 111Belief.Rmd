...
# Gender
pop_gender_prop <- tapply(pop$Count, pop$Gender, sum)
pop_gender_prop <- pop_gender_prop / sum(pop_gender_prop)
pop_gender_prop

# AgeBand
pop_age_prop <- tapply(pop$Count, pop$AgeBand, sum)
pop_age_prop <- pop_age_prop / sum(pop_age_prop)
pop_age_prop

# SES
pop_ses_prop <- tapply(pop$Count, pop$SES, sum)
pop_ses_prop <- pop_ses_prop / sum(pop_ses_prop)
pop_ses_prop



all_data$AgeBand <- NA
all_data$AgeBand[all_data$age_clean < 18] <- 1
all_data$AgeBand[all_data$age_clean >= 18 & all_data$age_clean < 30] <- 2
all_data$AgeBand[all_data$age_clean >= 30 & all_data$age_clean < 45] <- 3
all_data$AgeBand[all_data$age_clean >= 45 & all_data$age_clean < 60] <- 4
all_data$AgeBand[all_data$age_clean >= 60] <- 5



# Gender
sample_gender_prop <- prop.table(table(all_data$sex_clean))
sample_gender_prop

# AgeBand
sample_age_prop <- prop.table(table(all_data$AgeBand))
sample_age_prop

# SES
sample_ses_prop <- prop.table(table(all_data$ses5_clean))
sample_ses_prop
...
# Gender
cbind(
  Population = pop_gender_prop[names(sample_gender_prop)],
  Sample     = sample_gender_prop
)

# AgeBand
cbind(
  Population = pop_age_prop[names(sample_age_prop)],
  Sample     = sample_age_prop
)

# SES
cbind(
  Population = pop_ses_prop[names(sample_ses_prop)],
  Sample     = sample_ses_prop
)

```

Socioeconomic status distributions in the sample were broadly similar to those in the population. Compared with the population, individuals in SES level L4 were slightly over-represented in the sample, while those in SES level L3 were under-represented. The remaining SES groups showed very similar proportions between the sample and the population, suggesting that the sample is reasonably representative with respect to socioeconomic status.